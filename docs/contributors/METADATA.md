...
| `pageCount` (optional) | Positive-integer assertion for compatibility. Normally omit it: the build derives the canonical count from chunks separated by `<!-- pagebreak -->`. If supplied, it must equal that derived count. |
...
`version` 9, and `lastReviewed` 10 code points. The page count is generated by
the build rather than supplied as page-furniture metadata.
