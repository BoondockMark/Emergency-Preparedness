...
import { loadDocument, SECTIONS as sections } from './lib/metadata.mjs';
...
const files=(await walk(path.join(root,'handouts'))).filter(f=>f.endsWith('.md')).sort(),docs=[],codes=new Set();for(const file of files){const sourcePath=path.relative(root,file);const {data:d,content}=loadDocument(await fs.readFile(file,'utf8'),sourcePath);if(codes.has(d.code))throw Error(`${sourcePath}: code: duplicates ${d.code}`);codes.add(d.code);const chunks=content.split(/\n<!--\s*pagebreak\s*-->\n/i);if(chunks.length!==d.pageCount)throw Error(`${sourcePath}: pageCount: expected ${chunks.length}`);for(const attrs of imageReferences(content,d.code)){const ref=safeAssetPath(attrs.src,d.code);if(ref.code!==d.code)throw Error(`${d.code}: image assets must be stored in their own code directory, not ${ref.code}`);const entry=manifests.get(ref.code)?.get(ref.file);if(!entry)throw Error(`${d.code}: ${ref.file} is not declared in assets/handouts/${d.code}/manifest.json`);if(attrs.alt!==entry.alt)throw Error(`${d.code}/${ref.file}: HTML alt text must exactly match manifest alt text`)}docs.push({meta:d,chunks})}
