...
await fs.rm(path.join(root,'build'),{recursive:true,force:true});for(const d of ['build/html','build/assets/fonts','docs/pdfs','docs/previews'])await fs.mkdir(path.join(root,d),{recursive:true});
...
try{
  for(const {meta:m} of docs){
    ...
    const layout=await page.$$eval('.sheet',sheets=>sheets.map(sheet=>({scrollHeight:sheet.scrollHeight,clientHeight:sheet.clientHeight,scrollWidth:sheet.scrollWidth,clientWidth:sheet.clientWidth})));
    if(layout.length!==m.pageCount)throw Error(`${m.code}: HTML page count ${layout.length}, expected ${m.pageCount}`);
    layout.forEach((box,i)=>{if(box.scrollHeight>box.clientHeight||box.scrollWidth>box.clientWidth)throw Error(`${m.code} page ${i+1}: content overflow`)});
